...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        for x in range(len(player.deck.cards)):
            player.deck.cards[x].attack+=other_card.attack
            player.deck.cards[x].defense+=other_card.defense
        opponent.deck.cards=[x for x in opponent.deck.cards if x.attack!=other_card.attack or x.defense!=other_card.defense]
