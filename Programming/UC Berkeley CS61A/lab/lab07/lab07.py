...
def link_to_list(link):
    ...
    if link==Link.empty:
    	...
    	lst.append(link.first)
    	return lst+link_to_list(link.rest)
...
def cumulative_sum(t):
    ...
    def cumm(t):
    	if t.is_leaf():
    		return t.label
    	cum_sum=sum([cumm(b) for b in t.branches])
    	t.label+=cum_sum
    	return t.label
    cumm(t)
...
def bst_max(t):
	...
	def helper(t):
		...
		if t.label>maxx:
			maxx=t.label
		tp=[helper(b) for b in t.branches]
	helper(t)
	return maxx
...
def is_bst(t):
    ...
    if t.is_leaf():
    	...
    	if len(t.branches)==2:
    		lmax=bst_max(t.branches[0])
    		#print(lmax)
    		rmin=bst_min(t.branches[1])
    		#print(rmin)
    		if not (t.label>=lmax and t.label<rmin):
    			return False
