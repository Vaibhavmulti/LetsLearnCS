...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    return get_name(city1) if distance(make_city("Temp",lat,lon),city1) < distance(make_city("Temp",lat,lon),city2) else get_name(city2)
...
def sprout_leaves(t, vals):
    ...
    "*** YOUR CODE HERE ***"
    if is_leaf(t):
    	return tree(label(t),[[x] for x in vals])
    else:
...
def add_helper(t):
	if is_leaf(t):
		...
		br=[add_helper(b) for b in branches(t)]
		return tree(label(t),br)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
    	...
    	return tree(label(t1)+label(t2),[add_trees(b1,b2) for b1,b2 in list(zip_longest(branches(t1),branches(t2), fillvalue=tree(0)))])
    #print ((zip_longest(t1,t2, fillvalue=0)))
